...
@frappe.whitelist()
def get_logged_user(usr):
	print("---------------", usr)
	return frappe.session.user


@frappe.whitelist()
def sign(document_type_code, document):
	data = {}
	if document_type_code == "T02":  # ใบแจ้งหนี้/ใบกํากับภาษี
		data = prepare_T02(document)
	else:
		frappe.throw(_("Invalidate document type code - %s") % document_type_code)
	return {'XXX': 'YYY'}


def prepare_T02(document):
	data = {}
	print("-----------------", document.seller_tax_id)
	# currency_code = doc.currency_id.name
	# control = {
	# 	'auto_submit': 1,
	# 	'c01_seller_tax_id': doc.company_id.vat or '',
	# 	'c02_seller_branch_id': '00000' if (not doc.company_id.branch or doc.company_id.branch.lower() in ['head office', 'สำนักงานใหญ่']) else doc.company_id.branch or '',
	# 	'c03_file_name': '',
	# }
	# header = {
	...
	# 	'h23_source_system': self.env["ir.config_parameter"].sudo().get_param('web.base.url', ''),
